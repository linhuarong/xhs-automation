...
# Strict binding is local-only. It does not open shop, open Xiaohongshu, or call Yingdao OpenAPI.

if ($JobType -eq "search") {
  $endpoint = "$BaseUrl/api/workflows/xhs/account-binding/search/strict-check"
  $payload = @{
    job_id = $JobId
  ...
  $endpoint = "$BaseUrl/api/workflows/xhs/account-binding/publish/strict-check"
  $tagList = @()
...
Write-Host "strict_binding_input_path: $($result.strict_binding_input_path)"
Write-Host "strict_binding_result_path: $($result.strict_binding_result_path)"
Write-Host "strict_binding_summary_path: $($result.strict_binding_summary_path)"
