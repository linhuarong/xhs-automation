...
$body = @{
  ...
  job_id = $JobId
  account_id = if ([string]::IsNullOrWhiteSpace($AccountId)) { $null } else { $AccountId }
...
$response = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/workflows/xhs/n8n-handshake/ping" -ContentType "application/json; charset=utf-8" -Body ($body | ConvertTo-Json -Depth 10)

Write-Host "n8n ping handshake smoke completed."
