...
$body = @{
  ...
  payload = @{
    marker = "XHS_N8N_HANDSHAKE_SMOKE"
    dry_run = -not $RealHandshake
    keyword = "XHS_N8N_HANDSHAKE_SMOKE"
  }
}
...
$response = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/workflows/xhs/n8n-handshake/search" -ContentType "application/json; charset=utf-8" -Body ($body | ConvertTo-Json -Depth 10)

Write-Host "n8n search handshake smoke completed."
