...
def _find_first_element(driver: Any, selectors: list[str]) -> Any | None:
    """Find the first visible element from candidate selectors."""
    for selector in selectors:
        for element in _find_elements(driver, selector):
            is_displayed = getattr(element, "is_displayed", None)
            if is_displayed is None or is_displayed():
...
def search_xhs_keyword(
    ...
    try:
        ...
        if _has_visible_element(driver, LOGIN_OR_VERIFY_SELECTORS):
            screenshot_path = _safe_capture(provider, session, "search_waiting_human")
            log_job_event(
                job_id=job.job_id,
                step="login_or_verification_detected",
        ...
        search_input = _find_first_element(driver, SEARCH_INPUT_SELECTORS)
        ...
        if job.capture_screenshot:
            screenshot_path = provider.capture_screenshot(session, "search_result")
